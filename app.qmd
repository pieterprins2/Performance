...
wt_tickers_in_ETF_snapshot_date <- function(ETF = "WTCH_NA") {
  ...
  if(ETF == "IWDA_NA") {
    ...
    mutate(wt_tickers_in_ETF_snapshot_date = market_cap_eur/ticker_market_cap_snapshot_date("MSDEWIN"),
           ...
           wt_tickers_in_ETF_snapshot_date = ifelse(ticker %in% c(small_cap_tickers, "VFEM_NA"), 0, wt_tickers_in_ETF_snapshot_date))
  } else {
   sector <- 
...
```{r tabel_barchart_bijdrage_aan_trr_per_regel_vps}
#ON HOLD vanwege denkfout: gewicht in benchmark moet erbij?
#tabel met gewichten en trrs per datum per portefeuille voor de vp's
# tabel_weight_date_wtd_trrs_vps <-  
#   #tabel met gewogen er's per ticker per datum per portefeuille
#   portfolios_tabel |> 
#   #alleen RD, alleen vliegerportefeuilles
#   filter(RDRM == "RD", portfolio %in% portfolio_levels_vlieger) |> 
#   #bereken gewicht in RD per portefeuille per datum
#   group_by(portfolio, date) |> 
#   mutate(ticker_weight_in_RD = weight/sum(weight)) |> 
#   #weight op datum
#   rename(weight_date = date) |> 
#   #trr op de volgende datum
#   left_join(dates |> mutate(trr_date = lead(date, 1)), by = c("weight_date" = "date")) |> 
#   #trr erbij
#   left_join(all_trr_eur_long, by = c("ticker", "trr_date" = "date")) |> 
#   #MSDEWIN trr ernaast
#   left_join(MSDEWIN_trr, by = c("trr_date" = "date")) |>
#   #gewogen er per maand per modelportefeuille
#   #mutate(wtd_er_mtd = ticker_weight_in_RD * (trr - MSDEWIN_trr)) |> 
#   mutate(wtd_trr_mtd = ticker_weight_in_RD * trr) |> #- MSDEWIN_trr)) |> 
#   #kolommen selecteren en hernoemen
#   select(portfolio, ticker, weight_date, weight = ticker_weight_in_RD, date = trr_date, wtd_trr_mtd)
# 
# tabel_trr_mtd_ind_aand_vps <-
#   all_trr_eur_long |> filter(ticker %in% c(individuele_aandelen, etf_levels)) |>
#   #hulpkolom
#   mutate(portfolio = "Performance")
#   
# # bijdrage aan outperformance per regel
# tabel_gewogen_bijdrage_perf_per_regel <- function(start_date = last_ultimo_year, end_date = snapshot_date) {
#   # bijdrage aan (out-) performance per regel
#   datums <- dates$date[which(dates$date > start_date & dates$date <= end_date)]
#   
#  df_portfolios_tickers_wtd_trr <- 
#     tabel_weight_date_wtd_trrs_vps |> 
#     filter(portfolio %in% c("VP50", "VD50", "VP100")) |> 
#     #is group_by portfolio
#     filter(date %in% datums) |>
#     #group by portfolio en ticker (bij een portfolio maakt dat niet uit)
#     group_by(portfolio, ticker) |>
#     mutate(wtd_trr_mtd_cum = sum(wtd_trr_mtd)) |> 
#     reframe(trr = last(wtd_trr_mtd_cum)) # is de ctr, hernoemd tbv join later
# 
#   if(start_date < start_date_vd) {
#     #VD50 droppen als de startdatum eerder is dan die voor de VD50"
#     df_tickers_trr <-
#       tabel_trr_mtd_ind_aand_vps |>
#       group_by(ticker) |> 
#       #is group_by ticker
#       filter(date %in% datums) |>
#       mutate(trr_cum = cumprod(1 + trr) - 1) |>
#       reframe(trr = last(trr_cum),
#               portfolio = "Performance") |> 
#       select(portfolio, ticker, trr) |> 
#       #deze laatste tabel binden met de eerdere zodat alle vier kolommen erbij kunnen
#       bind_rows(df_portfolios_tickers_wtd_trr) |> 
#       #NA's verwijderen indien niet aanwezig in portefeuilles
#       pivot_wider(names_from = portfolio, values_from = trr) |> 
#       #hier gaat het om, helaas niet gelukt met een if in de pipe
#       filter(!((is.na(VP50)&is.na(VP100)))) 
#       } else {
#      df_tickers_trr <-
#       tabel_trr_mtd_ind_aand_vps |>
#       group_by(ticker) |>
...
#       pivot_wider(names_from = portfolio, values_from = trr) |>
#       #hier gaat het om, helaas niet gelukt met een if in de pipe
#       filter(!((is.na(VP50)&is.na(VP100)&is.na(VD50))))
#       }
# 
#     df_tickers_trr <-
#       df_tickers_trr |> 
...
#              trr = value) |> 
#       #holdings gegevens erbij voor sectoren
#       left_join(holdings |> select(ticker, sector), by = c("ticker")) |> 
#       left_join(sectors_colors, by = c("sector" = "ticker")) |>
#       #kleuren ernaast met een erbij voor "None"
#       mutate(fill = ifelse(sector == "None", "gray", fill))
# 
#   #totaal gewogen er RD uit deze tabel zelf halen
#   totaal_wtd_er_VD50 <- df_tickers_trr |> filter(portfolio == "VD50") |> summarise(sum_trr = sum(trr, na.rm = TRUE)) |> pull(sum_trr)
#   totaal_wtd_er_VP50 <- df_tickers_trr |> filter(portfolio == "VP50") |> summarise(sum_trr = sum(trr, na.rm = TRUE)) |> pull(sum_trr)
#   totaal_wtd_er_VP100 <- df_tickers_trr |> filter(portfolio == "VP100") |> summarise(sum_trr = sum(trr, na.rm = TRUE)) |> pull(sum_trr)
#   #regel_totaal maken om te binden aan de tabel
#   regel_totaal_VD50 <- tibble(
#     ticker = "TOTAAL",
#     portfolio = "VD50",
#     trr = totaal_wtd_er_VD50,
#     sector = "TOTAAL",
#     fill = portfolios_colors |> filter(portfolio == "VD50") |> pull(color)
#   )
#   regel_totaal_VP50 <- tibble(
#     ticker = "TOTAAL",
#     portfolio = "VP50",
#     trr = totaal_wtd_er_VP50,
#     sector = "TOTAAL",
#     fill = portfolios_colors |> filter(portfolio == "VP50") |> pull(color)
#   )
#   regel_totaal_VP100 <- tibble(
#     ticker = "TOTAAL",
#     portfolio = "VP100",
#     trr = totaal_wtd_er_VP100,
#     sector = "TOTAAL",
#     fill = portfolios_colors |> filter(portfolio == "VP100") |> pull(color)
#   )
#   regel_TOTAAL <- tibble(
#     ticker = "TOTAAL",
#     portfolio = "Performance",
#     trr = NA, #er is geen eenduidige portefeuille return
#     sector = "TOTAAL",
#     fill = portfolios_colors |> filter(portfolio == "VP100") |> pull(color)
#   )
# 
#   df <- 
#     df_tickers_trr # |> 
#   #  bind_rows(regel_totaal_VD50, regel_totaal_VP50, regel_totaal_VP100, regel_TOTAAL)
#  
#   if(start_date < start_date_vd) {
#     "VD50 droppen als de startdatum eerder is dan die voor de VD50"
#     df <- df |> filter(portfolio != "VD50")
#   }
#   
#   df <- df |> 
#     #voor nu alleen VP100 
#     filter(portfolio %in% c("Performance", "VP100")) |> 
#     # Set sector as factor with custom order from sector_levels
#      mutate(sector = factor(sector, levels = rev(c(sector_levels_RD, "None")))) |>
#     # # # Sort by sector (per sector_levels) and then by wtd_er (descending)
#      arrange(sector, desc(trr)) |>
#     # # Set ticker as factor to preserve the sorted order
#      mutate(ticker = factor(ticker, levels = unique(ticker)),
#             portfolio = factor(portfolio, levels = c("Performance", "VP100", "VP50", "VD50")))
#   df
# }
```

```{r barchart_bijdrage_aan_trr_per_regel_vps}

barchart_bijdrage_perf_per_regel <- function(start_date = last_ultimo_year, end_date = snapshot_date) {
  sel_ff_tabel_tickers_selectie_start_end(start_date = start_date, end_date = end_date) |> 
    left_join(tickers_fills_VPs, by = "ticker") |>
    mutate(abs_sel_ff = abs(sel_ff)) |>
    filter(abs_sel_ff > 0.0001) |>
    mutate(sector = factor(sector, levels = sector_levels_RD)) |>
    arrange(sector, desc(sel_ff)) |>
    ggplot(aes(x = ticker, y = sel_ff)) +
    coord_flip() + 
    geom_bar(aes(fill = fill), stat = "identity", col = "gray", alpha = 0.8) + 
    scale_fill_identity() +
    geom_text(aes(x = ticker, y = sel_ff/2, label = str_c(round(sel_ff*100, 1), "%")), 
              size = 3, col = "gray40", fontface = 'bold') +
    scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
    labs(x = "", 
         y = "", 
         title = str_c("Sel ff in RD van VP100, van ", format(as.Date(start_date), "%d-%b-%Y"), " t/m ", format(as.Date(end_date), "%d-%b-%Y")),
         caption = "rekening houdend met dividenden en gewichten in de loop der tijd") +
    theme_minimal() +
    theme(legend.position = "none",
          title = element_text(hjust = 0.5),
          #geen waarde op de as als alle bars al nummeers bevatten
...
<!-- #| fig-height: 7 -->
<!-- #| fig-width: 10 -->
<!-- barchart_bijdrage_perf_per_regel(start_date = last_ultimo_year, end_date = snapshot_date) -->
<!-- ``` -->
...
<!-- #| fig-height: 7 -->
<!-- #| fig-width: 10 -->
<!-- barchart_bijdrage_perf_per_regel(start_date = last_quarter_end, end_date = snapshot_date) -->
<!-- ``` -->
...
<!-- #| fig-height: 7 -->
<!-- #| fig-width: 10 -->
<!-- barchart_bijdrage_perf_per_regel(start_date = lastdate, end_date = snapshot_date) -->
<!-- ``` -->
...
<!-- #| fig-height: 7 -->
<!-- #| fig-width: 10 -->
<!-- barchart_bijdrage_perf_per_regel(start_date = next_to_lastdate, end_date = lastdate) -->
<!-- ``` -->
...
<!-- #| fig-height: 7 -->
<!-- #| fig-width: 10 -->
<!-- barchart_bijdrage_perf_per_regel(start_date = last_quarter_start, end_date = last_quarter_end) -->
<!--  # barchart_bijdrage_perf_per_regel(start_date = start_date_vp_vf, end_date = "2022-12-31") -->
<!--  # barchart_bijdrage_perf_per_regel(start_date = "2022-12-31", end_date = snapshot_date) -->
<!-- ``` -->
...
<!-- #| fig-height: 7 -->
<!-- #| fig-width: 10 -->
<!-- barchart_bijdrage_perf_per_regel(start_date = next_to_last_ultimo_year, end_date = last_ultimo_year) -->
<!-- ``` -->
...
<!-- #| fig-height: 7 -->
<!-- #| fig-width: 10 -->
<!-- barchart_bijdrage_perf_per_regel(start_date = next_to_last_ultimo_year2, end_date = next_to_last_ultimo_year) -->
<!-- ``` -->
...
<!-- #| fig-height: 7 -->
<!-- #| fig-width: 10 -->
<!-- barchart_bijdrage_perf_per_regel(start_date = start_date_vp_vf, end_date = snapshot_date) -->
<!-- ``` -->
